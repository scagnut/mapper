...
canvas.addEventListener("click", (e) => {
  ...
  if (x >= 0 && y >= 0 && x < GRID_WIDTH && y < GRID_HEIGHT) {
    if (selectedTile !== null) {
      ...
      linePath.push({ x, y });  // Add placed tile to the line path
      drawGrid();
    }
  }
});
...
document.addEventListener("keydown", (e) => {
  ...
  if (selectedTile !== null) {
    ...
    linePath.push({ ...currentPos });  // Add placed tile to the line path
    drawGrid();
  }

  // Place a line while holding Enter
  if (e.key === "Enter") {
    if (linePath.length === 0 || !linePath[linePath.length - 1].x === currentPos.x || linePath[linePath.length - 1].y === currentPos.y) {
